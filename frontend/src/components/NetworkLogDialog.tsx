      ...
      <div className="network-log-dialog__surface">
        <header className="network-log-dialog__header">
          <div className="network-log-dialog__title">
            ...
            <div>
              <h2 id="usage-record-title">使用记录</h2>
              <p>{visible.length} 条{!isTauri ? " · 浏览器示例" : ""}</p>
            </div>
          </div>
          <div className="network-log-dialog__actions">
            <button className="button button--secondary network-log-copy" type="button" onClick={() => void copyRows()}>
              {copyState === "done" ? <Check size={14} /> : <Copy size={14} />}
              {copyState === "done" ? "已复制" : copyState === "error" ? "复制失败" : "复制记录"}
            </button>
            <button ref={closeRef} className="network-log-close" type="button" aria-label="关闭" onClick={onClose}>
              <X size={18} />
            </button>
          </div>
        </header>
