...
/// Codex echoes this back within a turn; it belongs to the old account.
...
fn request(account: Option<&str>, state: &str) -> Request<Body> {
    let mut request = Request::builder()
        ...
        .header(TURN_STATE_HEADER, state);
...
#[tokio::test]
async fn account_switch_preserves_request_identity() {
    if std::env::var_os("CSK_ACCOUNT_SWITCH_TEST_CHILD").is_none() {
        let home = tempfile::tempdir().unwrap();
        let output = std::process::Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "proxy::account_switch_tests::account_switch_preserves_request_identity",
                "--nocapture",
            ])
    ...
    tokio::time::timeout(Duration::from_secs(15), async {
        ...
        let client_state = "client-turn-state";
        ...
        // No status polling yet: requests still pick up B's login at once.
        write_account(home.path(), "account-b");
        for client in [Some("account-a"), Some("account-b"), None] {
            let pending = tokio::spawn(proxy_http(app.clone(), request(client, client_state)));
            let (headers, reply) = requests.recv().await.unwrap();
