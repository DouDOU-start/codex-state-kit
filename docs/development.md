...
纯浏览器预览使用模拟接口，不会修改真实 Codex 配置或启动订阅内核。桌面开发版会执行真实操作，建议使用专门的 Codex 配置目录。

部分测试会在子进程中使用临时用户目录运行，其余测试使用内存数据库和临时目录；仍建议在隔离、可丢弃的用户环境执行全量测试，不要直接在日常账号目录运行。登录测试使用临时目录和本机模拟服务。
