...
这两项只统计本进程观测到的业务流量，按转发时生效的账号分开计数，切换账号不会混合；退出应用后清空。页面随状态轮询约每 1.5 秒更新。

下方「使用统计」按账号汇总近 30 天的数据：总成本、总请求、日均成本和请求、今日、成本最高的一天、累计 Tokens、平均耗时、活跃天数、每日用量趋势和模型分布。

## 使用记录

「使用记录」页按请求开始时间倒序列出记录，每页 50 条。可以按账号筛选，勾选「只看降智请求」只显示被标记的请求；「复制本页」把当前页复制为制表符分隔的文本。
...
| 费用 | 本次估算成本；悬停可看输入、缓存读、缓存写、输出各项；没有价格或用量不完整时显示「未定价」 |
