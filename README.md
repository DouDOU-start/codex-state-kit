...
- **虚拟设备**：转发时统一替换客户端身份（设备标识、版本、系统信息），账号之间互不混用；多窗口按客户端 session/window 稳定映射到独立虚拟作用域，任务并发和 `previous_response_id` 不会串线；关闭「启用虚拟设备模拟」后，设备和环境信息会原样透传。系统只能选 macOS、Windows 或 Linux，系统版本、架构和终端按官方 CLI 在该系统上的上报值固定，CLI 版本跟随本机 codex，避免拼出不存在的组合。
...
- **局域网转发**：可在不改变本机 Codex 自动接入的情况下开放同一端口供局域网客户端使用；远端通过 Kit API Key 鉴权，并沿用当前选中的 Kit 账号。
...
原生 WebSocket 客户端的业务请求走上游 WebSocket，和官方客户端一样；HTTP 客户端走 HTTP SSE，以确保流结束时能拿到完整 usage 并计算费用。正式版本机代理默认监听 `127.0.0.1:8787`。
